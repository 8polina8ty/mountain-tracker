...
export default function PublicUserProfilePage() {
  ...
  useEffect(() => {
    ...
    async function loadProfile() {
      if (!userId) {
        setErrorMessage("ID пользователя не найден.");
        setLoading(false);
        return;
      }
      ...
      <main className="flex min-h-[calc(100vh-80px)] items-center justify-center bg-gray-50 px-4">
        <section className="w-full max-w-lg rounded-3xl border border-gray-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-gray-500">
            Возможно, профиль был удалён или ссылка
            неправильная.
          </p>
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-6 overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
              ...
              <div className="text-white">
                ...
                <p className="mt-2 text-white/80">
                  Участник сообщества Mountain Tracker
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-4 sm:p-8">
            <ProfileStatistic
              icon="🏔️"
              label="Покорено вершин"
              value={profile.ascents_count}
            />

            <ProfileStatistic
              icon="📈"
              label="Суммарная высота"
              value={`${profile.total_height.toLocaleString(
                "ru-RU",
              )} м`}
            />
            ...
            <ProfileStatistic
              ...
              value={
                profile.highest_mountain_name
                  ? `${profile.highest_mountain_name} · ${profile.highest_mountain_height.toLocaleString(
                      "ru-RU",
                    )} м`
                  : "Нет данных"
              }
...
<section className="mt-6">
  <div className="mb-4">
    ...
    <h2 className="mt-2 text-2xl font-bold text-gray-900">
      Карта покорённых вершин
    </h2>

    <p className="mt-2 text-gray-500">
      Все публичные восхождения пользователя на карте.
    </p>
  </div>
  ...
  {ascents.length === 0 ? (
    <div className="mt-6 rounded-2xl bg-gray-50 p-8 text-center">
      ...
      <p className="mt-2 text-gray-500">
        Пользователь ещё не отметил покорённые вершины.
      </p>
    </div>
  ) : (
          ...
          >
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-500">
                <span>
                  ⛰ {ascent.mountain_height.toLocaleString("ru-RU")} м
                </span>
