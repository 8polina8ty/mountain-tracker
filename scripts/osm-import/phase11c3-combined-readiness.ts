...
export const PHASE11C3_COMBINED_READINESS_PATH = "data/osm/alps/publication/phase11c3-combined-readiness.json";
...
export interface Phase11C3ScaleReadiness {
  schemaVersion: 1;
  artifactType: "PHASE11C3_COMBINED_SCALE_READINESS";
  generatedAt: string;
  ...
  historicalReviewed: {
    ...
    eligibleByRouteType: Record<string, number>;
    blockedByRouteType: Record<string, number>;
  };
  phase11c4: {
    total: number;
    approved: number;
    ...
    eligibleByRouteType: Record<string, number>;
    blockedByRouteType: Record<string, number>;
    approvedButBlocked: number;
    approvedButBlockedReasons: Record<string, number>;
  };
  combinedReviewed: number;
  combinedVisuallyApproved: number;
  candidatePool: {
    totalStagingRoutes: number;
    totalReviewedRoutes: number;
    totalVisuallyApproved: number;
    ...
    eligibleNonActiveByRouteType: Record<string, number>;
    blockedAfterQaApprovalByReason: Record<string, number>;
  };
...
export interface Phase11C3QAExpansionRecord {
  ...
  sourceQueue: "historical" | "phase11c4";
}
...
export async function buildCombinedScaleReadiness(): Promise<Phase11C3ScaleReadiness> {
  ...
  const phase11c4Manifest = buildPublicationCandidateManifest(phase11c4Artifact);

  const baselineV1 = activeRows.filter(r => r.publication_contract_version === "mountain-tracker-osm-publication/v1");
  const baselineV2 = activeRows.filter(r => r.publication_contract_version === "mountain-tracker-osm-publication/v2");
  const baselineRelationIds = activeRows.map(r => r.canonical_relation_id).sort((a, b) => Number(a) - Number(b));
  const baselinePublicationIdempotencyKeys = activeRows.map(r => r.publication_idempotency_key).sort();

  const baselineSnapshot: Phase11C3BaselineSnapshot = {
    baselineActiveCount: activeRows.length,
  ...
  const allHistoricalCandidates = historicalArtifact.candidates;
  const allPhase11c4Candidates = phase11c4Artifact.candidates;
  const allCandidates = [...allHistoricalCandidates, ...allPhase11c4Candidates];

  const historicalTopologyDist: Record<string, number> = {};
  const historicalSemanticTypeDist: Record<string, number> = {};
  const historicalRouteTypeDist: Record<string, number> = {};
  let historicalEligibleNonActive = 0;
  let historicalBlockedCandidates = 0;

  const phase11c4TopologyDist: Record<string, number> = {};
  const phase11c4SemanticTypeDist: Record<string, number> = {};
  const phase11c4RouteTypeDist: Record<string, number> = {};
  let phase11c4EligibleNonActive = 0;
  let phase11c4BlockedCandidates = 0;
  ...
  const eligibleByRouteType: Record<string, number> = {};
  const blockedByRouteType: Record<string, number> = {};
  ...
  const historicalEligibleByRouteType: Record<string, number> = {};
  const historicalBlockedByRouteType: Record<string, number> = {};
  const phase11c4EligibleByRouteType: Record<string, number> = {};
  const phase11c4BlockedByRouteType: Record<string, number> = {};
  const phase11c4ApprovedButBlockedReasons: Record<string, number> = {};
  let phase11c4ApprovedButBlocked = 0;
  ...
  for (const candidate of allCandidates) {
    ...
    const sourceQueue = allHistoricalCandidates.includes(candidate) ? "historical" : "phase11c4";
    if (sourceQueue === "historical") {
      historicalTopologyDist[topology] = (historicalTopologyDist[topology] || 0) + 1;
      historicalSemanticTypeDist[candidate.semanticType] = (historicalSemanticTypeDist[candidate.semanticType] || 0) + 1;
      historicalRouteTypeDist[activity.routeType] = (historicalRouteTypeDist[activity.routeType] || 0) + 1;
    } else {
      phase11c4TopologyDist[topology] = (phase11c4TopologyDist[topology] || 0) + 1;
      phase11c4SemanticTypeDist[candidate.semanticType] = (phase11c4SemanticTypeDist[candidate.semanticType] || 0) + 1;
      phase11c4RouteTypeDist[activity.routeType] = (phase11c4RouteTypeDist[activity.routeType] || 0) + 1;
    }
    ...
    if (candidate.provenance.sourceUrl && allCandidates.some(c => c !== candidate && c.provenance.sourceUrl === candidate.provenance.sourceUrl)) {
    ...
    if (failures.length > 0) {
      blockedCandidates++;
      blockedByRouteType[activity.routeType] = (blockedByRouteType[activity.routeType] || 0) + 1;
      reasonForBlock = failures.join("; ");
      if (sourceQueue === "historical") {
        historicalBlockedCandidates++;
        historicalBlockedByRouteType[activity.routeType] = (historicalBlockedByRouteType[activity.routeType] || 0) + 1;
      } else {
        phase11c4BlockedCandidates++;
        phase11c4BlockedByRouteType[activity.routeType] = (phase11c4BlockedByRouteType[activity.routeType] || 0) + 1;
        // Check if this was an approved route that got blocked
        if (candidate.qaDecision.status === "VISUALLY_APPROVED") {
          phase11c4ApprovedButBlocked++;
          for (const failure of failures) {
            phase11c4ApprovedButBlockedReasons[failure] = (phase11c4ApprovedButBlockedReasons[failure] || 0) + 1;
          }
        }
      }
      ...
      if (sourceQueue === "historical") {
        ...
        if (candidate.qaDecision.status === "VISUALLY_APPROVED") {
          phase11c4ApprovedButBlocked++;
          phase11c4ApprovedButBlockedReasons[reasonForBlock] = (phase11c4ApprovedButBlockedReasons[reasonForBlock] || 0) + 1;
        }
      }
    } else {
      eligibleNonActive++;
      eligibleByRouteType[activity.routeType] = (eligibleByRouteType[activity.routeType] || 0) + 1;
      if (sourceQueue === "historical") {
        historicalEligibleNonActive++;
        historicalEligibleByRouteType[activity.routeType] = (historicalEligibleByRouteType[activity.routeType] || 0) + 1;
      } else {
        phase11c4EligibleNonActive++;
        phase11c4EligibleByRouteType[activity.routeType] = (phase11c4EligibleByRouteType[activity.routeType] || 0) + 1;
      }
      candidatePoolRecords.push({
        ...
        candidateManifestHash: sourceQueue === "historical" ? historicalManifest.overallDeterministicManifestHash : phase11c4Manifest.overallDeterministicManifestHash,
        ...
        qaHistoryHash: (() => {
          const relevantHistory = sourceQueue === "historical" ? historicalInput.qaHistory : phase11c4Input.qaHistory;
          const snapshot = relevantHistory
  ...
  const historicalReviewedRoutes = historicalInput.routes.length;
  const historicalVisuallyApproved = historicalArtifact.qaProgress.visuallyApproved;
  const historicalTotalCandidates = historicalArtifact.candidateCount;

  const phase11c4Total = phase11c4Input.routes.length;
  const phase11c4Approved = phase11c4Artifact.qaProgress.visuallyApproved;
  const phase11c4Rejected = phase11c4Artifact.qaProgress.rejected;
  const phase11c4Pending = phase11c4Artifact.qaProgress.pending;
  ...
  const candidatePool = {
    ...
    totalReviewedRoutes: historicalReviewedRoutes + phase11c4Total,
    totalVisuallyApproved: historicalVisuallyApproved + phase11c4Approved,
    ...
    eligibleNonActiveByRouteType: eligibleByRouteType,
    blockedAfterQaApprovalByReason: phase11c4ApprovedButBlockedReasons,
  };
  ...
  const readinessContent = {
    ...
    artifactType: "PHASE11C3_COMBINED_SCALE_READINESS" as const,
    generatedAt,
    baselineSnapshot,
    historicalReviewed: {
      totalStagingRoutes: historicalReviewedRoutes,
      totalReviewedRoutes: historicalReviewedRoutes,
      totalVisuallyApproved: historicalVisuallyApproved,
      totalCandidates: historicalTotalCandidates,
      eligibleNonActive: historicalEligibleNonActive,
      blockedCandidates: historicalBlockedCandidates,
      routeTypeDistribution: historicalRouteTypeDist,
      eligibleByRouteType: historicalEligibleByRouteType,
      blockedByRouteType: historicalBlockedByRouteType,
    },
    phase11c4: {
      total: phase11c4Total,
      approved: phase11c4Approved,
      rejected: phase11c4Rejected,
      pending: phase11c4Pending,
      needsReview: phase11c4Artifact.qaProgress.needsReview,
      eligibleNonActive: phase11c4EligibleNonActive,
      blockedCandidates: phase11c4BlockedCandidates,
      routeTypeDistribution: phase11c4RouteTypeDist,
      eligibleByRouteType: phase11c4EligibleByRouteType,
      blockedByRouteType: phase11c4BlockedByRouteType,
      approvedButBlocked: phase11c4ApprovedButBlocked,
      approvedButBlockedReasons: phase11c4ApprovedButBlockedReasons,
    },
    combinedReviewed: historicalReviewedRoutes + phase11c4Total,
    combinedVisuallyApproved: historicalVisuallyApproved + phase11c4Approved,
    candidatePool,
    qaExpansionQueue,
  };
