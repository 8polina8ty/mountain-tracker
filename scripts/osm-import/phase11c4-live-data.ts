...
export async function loadPhase11c4PublicationGateInput(): Promise<PublicationGateInput> {
  ...
  const [manifest, metadata, plan] = await Promise.all([
    readFile(resolve(STAGING_DIRECTORY, "phase11c4-staging-manifest.json"), "utf8").then(
      (value) => JSON.parse(value) as PreviewManifest,
    ),
    readFile(resolve(STAGING_DIRECTORY, "phase11c4-preview-metadata.json"), "utf8").then(
  ...
  if (routeResult.error) {
    throw new Error(`Phase 11C.4 staging query failed: ${routeResult.error.message}`);
  }
  ...
  if (summitResult.error) {
    throw new Error(`Phase 11C.4 summit query failed: ${summitResult.error.message}`);
  }
  if (qaResult.error) {
    throw new Error(`Phase 11C.4 QA query failed: ${qaResult.error.message}`);
  }
  if (historyResult.error) {
    throw new Error(`Phase 11C.4 QA history query failed: ${historyResult.error.message}`);
  }
